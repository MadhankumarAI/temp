...
adaptive mapping exists to avoid, reintroduced one stage later.
...
def cell_drivability(c, sensor_xy=(0.0, 0.0), res=g.res0 * (1 << DRIVE_LEVEL),
                     ...
                     min_pts=MIN_GROUND_PTS, sigma_sensor=0.0, terms=False):
    ...
    #
    # The sensor's own noise comes off the same way, and for the same reason.
    # Roughness is meant to be a property of the TERRAIN, and a ranging lidar
    # with standard deviation sigma adds sigma^2 to the height variance of a
    # perfectly flat surface. Leaving it in means the estimator cannot tell a
    # noisy sensor from a rough road: on the CARLA capture the lidar scatters
    # 0.074 m off a plane the semantic sensor sees as exactly flat, against a
    # 0.040 m limit, so every flat road graded marginal and drivable coverage
    # collapsed to 1.5%.
    #
    # Variances of independent contributions add, so they subtract. This is not
    # a threshold being loosened to get a nicer number: the limit is unchanged
    # and still 0.040 m of real bumpiness. What changes is that the quantity
    # being compared to it is bumpiness rather than bumpiness plus instrument.
    #
    # sigma_sensor defaults to 0, so nothing changes for a caller that does not
    # measure it. It is not a free parameter to tune -- it is a property of the
    # sensor, measurable by looking at what it reports for a surface known to
    # be flat, which is what integration/carla.py's range_noise check does.
    rough = np.sqrt(np.maximum(
        var - step * step / 12.0 - sigma_sensor * sigma_sensor, 0.0))
