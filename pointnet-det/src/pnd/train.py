...
def train(cfg: Config) -> dict:
    ...
    for ep in range(cfg.epochs):
        ...
        if rep["f1_fg"] > best:
            ...
            # The anchor table rides along with the weights. Without it a
            # checkpoint trained against measured anchors gets decoded at
            # inference against whatever the module default happens to be,
            # and the boxes come out scaled by the ratio between the two --
            # right centre, right yaw, wrong size, 0.00 AP, no error.
            from .dataset import ANCHORS
            torch.save({"model": keep.state_dict(), "cfg": cfg.to_dict(),
                        "anchors": np.asarray(ANCHORS).tolist(),
