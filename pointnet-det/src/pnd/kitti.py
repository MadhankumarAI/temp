...
# The first six are KITTI's and their indices are FROZEN: a checkpoint trained
# before the last four existed still loads, and its head still means what it
# meant. The last four are static classes KITTI has no labels for at all, and
# they exist because CARLA does label them -- poles, lights, signs, and one
# bucket for the built scene. A perception stack needs static objects as
# well as dynamic ones, and with six classes everything static was background.
#
# Appended, never reordered. Reordering would silently repoint every trained
# head, which is the same class of bug as the anchor mismatch below: nothing
# errors, the numbers just quietly stop meaning anything.
#
# A class list longer than the data is harmless. Van is empty in every CARLA
# capture and Pole is empty in every KITTI one; an empty class contributes no
# samples and its head simply never updates.
CLASSES = ["Background", "Car", "Pedestrian", "Cyclist", "Van", "Truck",
           "Pole", "TrafficLight", "TrafficSign", "Structure"]
