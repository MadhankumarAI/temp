...
Point classes exported, mapped onto three categories:
...
def _fov_mask(pts, fov_deg, max_range, out):
    """Which points the classifier is allowed to look at.

    The network was trained front-camera-only, so points outside that cone are
    not in its envelope. Written as one pass because the three temporaries the
    numpy form allocated (range, azimuth, and their comparison) were never read
    again, and the whole test collapses to two branches per point.

    Not fastmath: this is a threshold test, and reordering the operands could
    move a point across the boundary. The bins it feeds are discrete.
    """
    for i in prange(pts.shape[0]):
        x = pts[i, 0]
        y = pts[i, 1]
        r = math.sqrt(x * x + y * y)
        if r >= max_range or r <= 1.0:
            out[i] = False
        else:
            az = math.degrees(math.atan2(y, x))
            out[i] = -fov_deg <= az <= fov_deg
...
def process(pts, cfg, model, device, tracker=None, ground=None, terrain=True):
    ...
    t = {}
    N = len(pts)
    # `rng` and `az` existed only to build this one mask, and cost 9.7 ms to
    # do it: np.linalg.norm alone was 6 ms on 124k points, being a generic
    # reduction where a fused sqrt would do. Nothing downstream reads either
    # array, so neither is materialised now.
    in_fov = np.empty(len(pts), np.bool_)
    _fov_mask(pts, FOV_DEG, cfg.max_range, in_fov)
    ...
    if len(idx_work) >= 50:
        ...
        if n_clusters > 0:
            # Group the points by cluster in one pass. `lab == k` inside
            # the loop rescans the whole working set once per cluster, which is
            # O(clusters x points): 200 clusters over 30k points is 6M
            # comparisons to recover a partition the labels already describe.
            # A stable sort puts each cluster in a contiguous, ascending slice,
            # which is exactly what flatnonzero returned, so the sampling below
            # draws the same points in the same order.
            order = np.argsort(lab, kind="stable")
            first = np.searchsorted(lab[order], np.arange(n_clusters + 1))

            P, members = [], []
            for k in range(n_clusters):
                ii = order[first[k]:first[k + 1]]
                ...
                P.append(np.concatenate([op[sel, :4], oa[sel, None]], 1))
                members.append(ii)
            ...
            if P:
                ...
                # One transfer, not four. Every .cpu() is a blocking
                # sync, and four of them on tensors this small cost far more in
                # round trips than the arithmetic they carry: profiled at 43 ms
                # a sweep against 4 ms for the whole network. Packing the heads
                # into one contiguous tensor first pays for exactly one stall.
                hb = out["head_bin"].float().argmax(1)
                hr = out["head_res"].float().gather(1, hb.unsqueeze(1)).squeeze(1)
                nc = out["logits"].shape[1]
                packed = torch.cat([
                    out["logits"].float().softmax(1),
                    out["center"].float(),
                    out["size_log"].float(),
                    decode_heading(hb.float(), hr).unsqueeze(1)],
                    dim=1).cpu().numpy()
                prob = packed[:, :nc]
                dc = packed[:, nc:nc + 3]
                sl = packed[:, nc + 3:nc + 6]
