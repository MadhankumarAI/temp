...
def _sector_of(pts, n_radial, n_azimuth, max_range, sec):
    """Polar sector index per point, in one pass.

    The numpy version of this was eight passes over the cloud (two dtype
    casts, a hypot, an arctan2, a sqrt, a clip and two int casts), each
    allocating its own 1 MB temporary. Measured at 14 ms of ground's 31 ms,
    and none of it arithmetic-bound: it was reading and writing the same
    124k points eight times. Fused, it is one read.

    No fastmath here. This decides which bin a point falls in, and reordering
    the float ops could move a point on a bin boundary. The plane fit that
    consumes it is fastmath, because there a last-bit difference is noise;
    here it is a different answer. Arithmetic is promoted to float64 for the
    same reason: it is what the numpy version did, so the bins are identical.
    """
    two_pi = 2.0 * math.pi
    for i in prange(pts.shape[0]):
        x = np.float64(pts[i, 0])
        y = np.float64(pts[i, 1])
        r = math.sqrt(x * x + y * y)
        if r > max_range:
            sec[i] = -1
            continue
        rn = math.sqrt(r / max_range)
        if rn > 0.999:
            rn = 0.999
        elif rn < 0.0:
            rn = 0.0
        ai = int((math.atan2(y, x) + math.pi) / two_pi * n_azimuth)
        if ai < 0:
            ai = 0
        elif ai >= n_azimuth:
            ai = n_azimuth - 1
        sec[i] = int(rn * n_radial) * n_azimuth + ai


@njit(cache=True)
def _bucket_order(sec_v, starts, order):
    """Stable counting sort of point indices by sector.

    argsort(kind="stable") costs 3.8 ms on 124k points because it is O(N log N)
    and general. The bucket bounds are already known -- `starts` came from the
    bincount two lines up -- so this is one O(N) scatter, and it emits indices
    in ascending order within each bucket, which is exactly what stable means.
    """
    pos = starts[:-1].copy()
    for i in range(sec_v.shape[0]):
        b = sec_v[i]
        order[pos[b]] = i
        pos[b] += 1
...
def remove_ground(pts: np.ndarray,
    ...
    sec = np.empty(len(pts), np.int64)
    _sector_of(pts, n_radial, n_azimuth, max_range, sec)
    ...
    sec_v = np.where(sec >= 0, sec, n_sec)       # out-of-range -> overflow bin
    counts = np.bincount(sec_v, minlength=n_sec + 1)
    starts = np.zeros(n_sec + 2, np.int64)
    np.cumsum(counts, out=starts[1:])
    order_by_sec = np.empty(len(sec_v), np.int64)
    _bucket_order(sec_v, starts, order_by_sec)
