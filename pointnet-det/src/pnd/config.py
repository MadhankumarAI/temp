...
class Config:
    ...
    val_frac: float = 0.2
    # A held-out third split, reported once rather than tuned against. 0
    # disables it, which is what a KITTI run wants: that benchmark has its own
    # official test split and this one would only shrink training.
    test_frac: float = 0.0
    # How many contiguous chunks the frames are cut into before splits are
    # drawn. More blocks means finer granularity and a split closer to the
    # requested fraction; fewer means each held-out stretch is longer and less
    # correlated with training. 20 over a couple of thousand frames gives
    # ~100-frame stretches, roughly 10 s of driving each.
    split_blocks: int = 20
    ...
    # --- model ------------------------------------------------------- #
    canon: str = "pca2_yaw"
    # Derived from the class list, never written as a literal. This was a hard
    # 6 while CLASSES grew to 10, so the head came out six wide and the four
    # new classes had no output to be predicted on. A count and the list it
    # counts must not be two separate facts.
